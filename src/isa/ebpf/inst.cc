...
void inst::insert_jmp_opcodes(unordered_set<int>& jmp_set) const {
  ...
  jmp_set.insert(IDX_JLEXC);
  jmp_set.insert(IDX_JLEXY);
  jmp_set.insert(IDX_JSETXC);
  jmp_set.insert(IDX_JSETXY);
  jmp_set.insert(IDX_JNEXC);
  jmp_set.insert(IDX_JNEXY);
  jmp_set.insert(IDX_JSGTXC);
  jmp_set.insert(IDX_JSGTXY);
  jmp_set.insert(IDX_JSGEXC);
  jmp_set.insert(IDX_JSGEXY);
  jmp_set.insert(IDX_JSLTXC);
  jmp_set.insert(IDX_JSLTXY);
  jmp_set.insert(IDX_JSLEXC);
...
z3::expr inst::smt_inst_jmp(smt_var & sv) const {
  ...
  switch (_opcode) {
    ...
    case JLEXC: return (ule(CURDST, IMM));
    case JLEXY: return (ule(CURDST, CURSRC));
    case JSETXC: return (CURDST & IMM);
    case JSETXY: return (CURDST & CURSRC);
    case JNEXC: return (CURDST != IMM);
    case JNEXY: return (CURDST != CURSRC);
    case JSGTXC: return (CURDST > IMM);
    case JSGTXY: return (CURDST > CURSRC);
    case JSGEXC: return (CURDST >= IMM);
    case JSGEXY: return (CURDST >= CURSRC);
    case JSLTXC: return (CURDST < IMM);
    case JSLTXY: return (CURDST < CURSRC);
    case JSLEXC: return (CURDST <= IMM);
...
int opcode_2_idx(int opcode) {
  switch (opcode) {
    ...
    case JLEXC: return IDX_JLEXC;
    case JLEXY: return IDX_JLEXY;
    case JSETXC: return IDX_JSETXC;
    case JSETXY: return IDX_JSETXY;
    case JNEXC: return IDX_JNEXC;
    case JNEXY: return IDX_JNEXY;
    case JSGTXC: return IDX_JSGTXC;
    case JSGTXY: return IDX_JSGTXY;
    case JSGEXC: return IDX_JSGEXC;
    case JSGEXY: return IDX_JSGEXY;
    case JSLTXC: return IDX_JSLTXC;
    case JSLTXY: return IDX_JSLTXY;
    case JSLEXC: return IDX_JSLEXC;
...
void inst::regs_to_read(vector<int>& regs) const {
  ...
  switch (_opcode) {
    ...
    case JLEXC:    regs = {_dst_reg}; return;
    case JLEXY:    regs = {_dst_reg, _src_reg}; return;
    case JSETXC:   regs = {_dst_reg}; return;
    case JSETXY:   regs = {_dst_reg, _src_reg}; return;
    ...
    case JSGEXC:   regs = {_dst_reg}; return;
    case JSGEXY:   regs = {_dst_reg, _src_reg}; return;
    case JSLTXC:   regs = {_dst_reg}; return;
    case JSLTXY:   regs = {_dst_reg, _src_reg}; return;
    case JSLEXC:   regs = {_dst_reg}; return;
...
#define SRC_ID (insn->_src_reg)
  ...
  static void *jumptable[NUM_INSTR] = {
    ...
    [IDX_JLEXC]    = && INSN_JLEXC,
    [IDX_JLEXY]    = && INSN_JLEXY,
    [IDX_JSETXC]   = && INSN_JSETXC,
    [IDX_JSETXY]   = && INSN_JSETXY,
    [IDX_JNEXC]    = && INSN_JNEXC,
    [IDX_JNEXY]    = && INSN_JNEXY,
    [IDX_JSGTXC]   = && INSN_JSGTXC,
    [IDX_JSGTXY]   = && INSN_JSGTXY,
    [IDX_JSGEXC]   = && INSN_JSGEXC,
    [IDX_JSGEXY]   = && INSN_JSGEXY,
    [IDX_JSLTXC]   = && INSN_JSLTXC,
    [IDX_JSLTXY]   = && INSN_JSLTXY,
    [IDX_JSLEXC]   = && INSN_JSLEXC,
...
#define COND_JMP(OPCODE, OP, DST_REG, SRC_REG, IMM_NUM)            \
  ...
  COND_JMP(JSET, &, UDST, USRC, UIMM)
  COND_JMP(JNE, !=, UDST, USRC, UIMM)
  COND_JMP(JSGT, >, DST, SRC, IMM)
  COND_JMP(JSGE, >=, DST, SRC, IMM)
  COND_JMP(JSLT, <, DST, SRC, IMM)
  COND_JMP(JSLE, <, DST, SRC, IMM)
#undef COND_JMP
