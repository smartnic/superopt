...
int spawn_server(int port) {
  // cout << "Hello before sleep" << endl;
  // sleep(10);
  // cout << "after sleep" << endl;
  cout << "Spawining Server with port: " << port << "" << endl;
  pid = fork();
  if (pid == -1) {
...
void read_from_solver(int sock) {
  ...
  if (total_read >= RESULT_SIZE_BYTES)
    cout << "Exhausted result read buffer" << endl;
  close(sock);
}
...
string write_problem_to_z3server(string formula) {
  // cout << "z3client: Received a formula to solve" << endl;
  ...
  if (no_child_now || time_to_respawn) {
    ...
    if (child_pid_1 <= 0) { /* unsuccessful spawn */
      cout << "z3client: spawning server 1 failed" << endl;
      return "";
    }
  ...
  if (server1_read > 0 && server2_read > 0) { /* both sockets are readable */
    ...
    if (server2_read > 0) {
      ...
      cout << "z3client: Only server 1 returned. Killing server 2" << endl;
      kill(child_pid_2, SIGKILL);
    ...
    if (server1_read > 0) {
      ...
      cout << "z3client: Only server 2 returned. Killing server 1" << endl;
      kill(child_pid_1, SIGKILL);
