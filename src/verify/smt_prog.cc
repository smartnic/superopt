...
// extend the path_con[cur_bid] with "p_con", and add the extended path condtion into path_con[next_bid]
void smt_prog::add_path_cond(expr p_con, size_t cur_bid, size_t next_bid) {
  ...
  for (size_t i = 0; i < path_con[next_bid].size(); i++) {
    if (g.nodes_in[next_bid][i] == cur_bid) {
      idx = i;
      break;
    }
  }
  ...
  for (size_t i = 0; i < path_con[cur_bid].size(); i++) {
    for (size_t j = 0; j < path_con[cur_bid][i].size(); j++) {
      path_con[next_bid][idx].push_back(path_con[cur_bid][i][j] && p_con);
    }
  }
}
