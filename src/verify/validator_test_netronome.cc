...
void test3() {
  ...
  inst p3[] {
    inst(IMMED, 16, 10),
    inst(ALU, 1, 0, ALU_AND_INV, 16),
    inst(ALU, 2, 0, ALU_INV_AND, 16),
    inst(ALU, 0, 1, ALU_MINUS, 2),
    inst(NOP),
  };
  ...
  result = vld.is_equal_to(p1, length, p3, length);
  cout << vld._store_f << endl;
  print_test_res(result, "p1 == p3");
  if (!result) {
    inout counterex = vld._last_counterex;
    cout << "counterexample: input=" << counterex.input.reg << " output=" << counterex.output.reg << endl;
  }
