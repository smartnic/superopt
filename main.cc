...
void usage()
{
  ...
  cout << "usage: " << endl
       ...
       << setw(W) << "-m"
       << ": turn on measurement" << endl
       << setw(W) << "--path_out arg"
       << ": output file path" << endl
       << endl
       << para_bm_desc() << endl
       << setw(W) << "--bm arg"
       << ": benchmark ID. toy_isa: 0 - 2; ebpf: 0 - 2" << endl
       << setw(W) << "--bm_from_file"
       << ": benchmark from file flag. the default is false" << endl
       << setw(W) << "--bytecode arg"
       << ": bpf bytecode file" << endl
       << setw(W) << "--desc arg"
       << ": bpf bytecode description file" << endl
       << setw(W) << "--w_e arg"
       << ": weight of error cost in cost function" << endl
       << setw(W) << "--w_p arg"
       << ": weight of performance cost in cost function" << endl
       << setw(W) << "--st_ex arg"
       << ": " + para_st_ex_desc() << endl
       << setw(W) << "--st_eq arg"
       << ": " + para_st_eq_desc() << endl
       << setw(W) << "--st_avg arg"
       << ": " + para_st_avg_desc() << endl
       << setw(W) << "--st_perf arg"
       << ": " + para_st_perf_desc() << endl
       << endl
       << para_restart_desc() << endl
       << setw(W) << "--st_when_to_restart arg"
       << ": " + para_st_when_to_restart_desc() << endl
       << setw(W) << "--st_when_to_restart_niter arg"
       << ": " + para_st_when_to_restart_niter_desc() << endl
       << setw(W) << "--st_start_prog arg"
       << ": " << para_st_start_prog_desc() << endl
       << setw(W) << "--restart_w_e_list arg"
       << ": " << para_restart_w_e_list_desc() << endl
       << setw(W) << "--restart_w_p_list arg"
       << ": " << para_restart_w_p_list_desc() << endl
       << endl
       << para_win_desc() << endl
       << setw(W) << "--reset_win_niter arg"
       << ": " + para_reset_win_niter_desc() << endl
       << setw(W) << "--win_s_list arg"
       << ": " + para_win_s_list_desc() << endl
       << setw(W) << "--win_e_list arg"
       << ": " + para_win_e_list_desc() << endl
       << endl
       << para_next_proposal_desc() << endl
       << setw(W) << "--p_inst_operand arg:"
       << ": " << para_p_inst_operand_desc() << endl
       << setw(W) << "--p_inst arg"
       << ": " << para_p_inst_desc() << endl
       << setw(W) << "--port arg"
       << ": " << para_port_desc() << endl;
}
...
bool parse_input(int argc, char *argv[], input_paras &in_para)
{
                            ...
                            long_opts, nullptr)) != -1)
  {
    ...
    case 'h':
      ...
      return false;
    case 'm':
      in_para.meas_mode = true;
      break;
    case 'n':
      in_para.niter = stoi(optarg);
      break;
    case 0:
      in_para.path_out = optarg;
      break;
    case 1:
      in_para.bm = stoi(optarg);
      break;
    case 2:
      in_para.bm_from_file = true;
      break;
    case 3:
      in_para.bytecode = optarg;
      break;
    case 4:
      in_para.desc = optarg;
      break;
    case 5:
      in_para.w_e = stod(optarg);
      break;
    case 6:
      in_para.w_p = stod(optarg);
      break;
    case 7:
      in_para.st_ex = stoi(optarg);
      break;
    case 8:
      in_para.st_eq = stoi(optarg);
      break;
    case 9:
      in_para.st_avg = stoi(optarg);
      break;
    case 10:
      in_para.st_perf = stoi(optarg);
      break;
    case 11:
      in_para.st_when_to_restart = stoi(optarg);
      break;
    case 12:
      in_para.st_when_to_restart_niter = stoi(optarg);
      break;
    case 13:
      in_para.st_start_prog = stoi(optarg);
      break;
    case 14:
      set_w_list(in_para.restart_w_e_list, optarg);
      break;
    case 15:
      set_w_list(in_para.restart_w_p_list, optarg);
      break;
    case 16:
      in_para.reset_win_niter = stoi(optarg);
      break;
    case 17:
      set_win_list(in_para.win_s_list, optarg);
      break;
    case 18:
      set_win_list(in_para.win_e_list, optarg);
      break;
    case 19:
      in_para.p_inst_operand = stod(optarg);
      break;
    case 20:
      in_para.p_inst = stod(optarg);
      break;
    case 21:
      PORT = stoi(optarg);
      cout << "Set Port to: " << PORT << "\n";
      break;
    case '?':
      usage();
      return false;
...
int main(int argc, char *argv[])
{
  ...
  if (!parse_input(argc, argv, in_para))
    return 0;
