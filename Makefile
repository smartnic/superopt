...
BPF_BENCH_FLAG=-D ISA_BPF_BENCH

all: main.out main_ebpf.out proposals_test.out inst_codegen_test_toy_isa.out inst_codegen_test_ebpf.out inst_test.out cost_test.out prog_test.out mh_prog_test.out validator_test.out cfg_test.out inout_test.out smt_prog_test.out ebpf_inst_test.out validator_test_ebpf.out cfg_test_ebpf.out bpf_bench_linux.out
...
bpf_bench_linux.out: main.cc main.h bpf_bench_z3.o measure/bpf_bench_linux.cc measure/bpf_bench_linux.h measure/meas_mh_bhv.h measure/meas_mh_bhv.cc $(SEARCH)mh_prog.cc $(SEARCH)mh_prog.h $(SEARCH)proposals.cc $(SEARCH)proposals.h $(ISA)prog.cc $(ISA)prog.h $(SEARCH)cost.cc $(SEARCH)cost.h $(SRC)inout.cc $(SRC)inout.h $(EBPF)inst_codegen.h $(ISA)inst_header.h $(ISA)inst.cc $(ISA)inst.h $(EBPF)inst.cc $(EBPF)inst.h $(VERIFY)validator.cc $(VERIFY)validator.h $(VERIFY)cfg.cc $(VERIFY)cfg.h $(VERIFY)smt_prog.cc $(VERIFY)smt_prog.h $(VERIFY)smt_var.cc $(VERIFY)smt_var.h $(SRC)utils.cc $(SRC)utils.h
	g++ $(EBPF_FLAG) $(BPF_BENCH_FLAG) -std=c++11 main_bpf_bench_z3.o measure/bpf_bench_linux.cc measure/meas_mh_bhv.cc $(ISA)inst.cc $(EBPF)inst.cc $(SEARCH)mh_prog.cc $(SEARCH)proposals.cc $(ISA)prog.cc $(SEARCH)cost.cc $(SRC)inout.cc $(VERIFY)validator.cc $(VERIFY)cfg.cc $(VERIFY)smt_prog.cc $(VERIFY)smt_var.cc $(SRC)utils.cc -o bpf_bench_linux.out ../z3/build/libz3$(SO_EXT) $(LINK_EXTRA_FLAGS)

bpf_bench_z3.o: main.cc
	$(CXX) $(EBPF_FLAG) $(BPF_BENCH_FLAG) $(CXXFLAGS) $(OS_DEFINES) $(EXAMP_DEBUG_FLAG) $(CXX_OUT_FLAG)main_bpf_bench_z3.o  -I../z3/src/api -I../z3/src/api/c++ main.cc
