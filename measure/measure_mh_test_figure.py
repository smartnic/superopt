...
def gen_nodes_info(raw_nodes, nodes, data):
    ...
    # print("nodes_info: ", nodes_info)
    # TODO: compute #unique program count and store it into node_info
    return nodes_info

# according to the fmt generated by function `gen_nodes_info`
def get_node_graph_name(node_name, info):
    return (node_name + "\n" +
            str(info[0]) + " " + str(info[5]) + "\n" +
            str(info[3]) + " " + str(info[4]))
...
def gen_edges_theoretical(nodes, nodes_info):
    edges = []
    for ns in nodes:
        for n in ns:
            v = nodes_info[n]
            n_name = get_node_graph_name(n, v)
            for i, b in enumerate(n):
                if b == '0':
                    x = n[:i] + '1' + n[i + 1:]
                    if x in nodes_info:
                        v = nodes_info[x]
                        x_name = get_node_graph_name(x, v)
                        edges.append((n_name, x_name))
    return edges

# edges_dic: [edge_name]:[edge_frequency_count]
def gen_edges_real(nodes_info, node_transfer_list):
    dic = {}
    n1 = node_transfer_list[0]
    for n in node_transfer_list[1:]:
        if n1 != n:
            e = (get_node_graph_name(n1, nodes_info[n1]),
                 get_node_graph_name(n, nodes_info[n]))
            if e in dic:
                dic[e] += 1
            else:
                dic[e] = 1
...
def draw_transfer_graph(fin_path, fout_path, para, id, type):
    ...
    nodes_info = gen_nodes_info(raw_nodes, nodes, data)
    node_color = gen_node_color(nodes_info)
    # edges = gen_edges_theoretical(nodes, nodes_info)
    edges, edge_color = gen_edges_real(nodes_info, data[line])
    ...
    for k, v in nodes_info.items():
        nodes_name.append(get_node_graph_name(k, v))
        g.add_node(get_node_graph_name(k, v), pos=get_node_pos(v))
